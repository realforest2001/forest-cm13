...
/obj/effect/step_trigger/ares_alert/access_control/Crossed(atom/passer as mob|obj)
	...
	if(check_contents)
		...
		if(!idcard)
			for(var/obj/item/holder in passer.contents)//For some reason locate() fails on anything that is force-moved onto a mob,
				idcard = locate(/obj/item/card/id) in holder.contents//like their spawn-in presets or from vendors.
				if(idcard)
					continue
	if(!istype(idcard))
